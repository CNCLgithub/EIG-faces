...
class EIG_classifier(nn.Module):
    def __init__(self, z_size=404, num_classes=25):
        super(EIG_classifier, self).__init__()
        ...
        self.vision_module_fc = nn.Sequential(*list(self.vision_module.children())[-9:-5]) #SFCL
        ...
        for p,k in enumerate(self.vision_module_fc.parameters()):
            k.requires_grad = False

        # Only train the top two layers for f3.
        self.fc_layers = FCLayers() #SFCL
        self.classifier = FCLayers(z_size, num_classes) # Identity layer

    def forward(self, x, segment=False, add_offset=False, test=False):
        dtype = torch.FloatTensor        
        if segment == True:
